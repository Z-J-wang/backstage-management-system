...
<script>
import CreateNewExperience from '@/modules/person-management/components/job_experience/CreateJob.vue'; // item 编辑器
import EditExperience from '@/modules/person-management/components/job_experience/EditJob.vue'; // item 编辑器
import CheckExperience from '@/modules/person-management/components/job_experience/CheckJob.vue'; // item 查看器
...
export default {
  ...
  methods: {
     ...
     */
    async deleteJob(id) {
      let res = await this.$HttpApi.deleteJob({ id: id });
      let data = {};
      if (res.status === 200 && res.data.code === 1000) {
        data = res.data.msg;
      } else {

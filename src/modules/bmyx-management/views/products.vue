  ...
  <div class="tableContain">
    <div class="table_tool clearfix">
      <div class="search float-l">
        <el-input v-model="search.text" class="input-with-select" placeholder="请输入搜索关键字">
          <el-select v-model="search.type" slot="prepend" placeholder="请选择类别">
            <el-option selected value="name" label="商品名称"></el-option>
            <el-option value="sort" label="商品类别"></el-option>
          </el-select>
    ...
    <el-table border stripe style="width: 100%" :data="dataList" highlight-current-row>
      <el-table-column sortable prop="id" label="id"></el-table-column>
      <el-table-column sortable prop="name" width="110" label="商品名称"></el-table-column>
      <el-table-column sortable label="商品类别">
      ...
      <el-table-column label="商品图片" prop="imgSrcList">
        <template slot-scope="scope">
          <el-image
      ...
      <el-table-column width="240" prop="detail" label="商品介绍"></el-table-column>
      <el-table-column prop="ban" width="150" label="状态">
        <template slot-scope="scope">
          <div v-if="!scope.row.ban" class="text-c ban">
            <el-tag class="tag" effect="dark" type="success">上架成功</el-tag>
            <el-switch
      ...
      <el-table-column sortable width="101" label="近期修改" prop="updatedAt"></el-table-column>
    ...
    <check-product :drawer-visible="itemCheckVisible" :item="itemValue" @close="closeItemCheck" />

    <creat-new-product :visible="addItemVisible" @close="closeAddItem" />
...
<script>
import EditProduct from '@/modules/bmyx-management/components/products/EditProduct.vue';
import CheckProduct from '@/modules/bmyx-management/components/products/CheckProduct.vue';
import CreatNewProduct from '@/modules/bmyx-management/components/products/CreatNewProduct.vue';
  ...
  methods: {
    ...
    async searchByNameOrSort(size, currentPage, search, isfirst = false) {
      let params = Object.assign(
        {
          pageSize: size || this.pagination.size,
          start: isfirst ? 0 : (currentPage - 1) * this.pagination.size + this.dataList.length || 0
        },
        search
      );
